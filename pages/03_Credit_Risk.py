...
with tab1:
    ...
    summary = (
        credit_df.groupby("rating")
        .agg({"exposure": "sum", "expected_loss": "sum", "borrower_id": "count"})
        .reset_index()
    )
    ...
    with col1:
        fig_exp = px.bar(
            ...
            y="exposure",
            text_auto=".2s",
            title="Total Exposure by Rating",
...
with tab2:
    ...
    with col2:
        fig_pie = px.pie(
            values=conc["exposures"].values,
            names=conc["exposures"].index,
            hole=0.4,
            template="plotly_dark",
            title="Exposure Breakdown by Sector",
        )
...
with tab4:
    ...
    selected_borrower = st.selectbox(
        "Select a Borrower ID to investigate", credit_df["borrower_id"].tolist()
    )
