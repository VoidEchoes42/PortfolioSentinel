...
st.markdown(
    "Inspect raw data pipelines, export datasets, and view macroeconomic indicators."
)

tab1, tab2, tab3 = st.tabs(
    ["Market Data", "Credit Portfolio", "Macro Indicators (FRED)"]
)

with tab1:
    market_df = get_market_data()
    st.subheader(
        f"Market Data ({market_df.index.min().date()} to {market_df.index.max().date()})"
    )
...
with tab2:
    ...
    st.download_button(
        "Download Credit Data CSV",
        data=csv2,
        file_name="credit_portfolio.csv",
        mime="text/csv",
    )
